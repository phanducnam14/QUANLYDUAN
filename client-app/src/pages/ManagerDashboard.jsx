    ...
    return (
        <div className="min-vh-100 bg-light d-flex flex-column" style={{ fontFamily: "'Segoe UI', sans-serif" }}>
                                    ...
                                    <div className="card border-0 shadow-sm h-100 hover-shadow transition" style={{cursor: 'pointer'}} onClick={() => handleSelectProject(p)}>
                                        <div className={`card-body ${p.status === 'CLOSED' ? 'bg-secondary bg-opacity-10' : ''}`}>
                                            ...
                                            <div className="d-flex align-items-center justify-content-between border-top pt-3">
                                                <div className="d-flex align-items-center"><div className="bg-light rounded-circle text-center small fw-bold text-secondary me-1" style={{width: 30, height: 30, lineHeight:'30px'}}>{p.members?.length || 0}</div><small className="text-muted">thành viên</small></div>
                            ...
                            <div className="card border-0 shadow-lg overflow-hidden">
                                <div className={`card-header p-4 border-bottom ${isProjectClosed ? 'bg-secondary text-white' : 'bg-white'}`}>
                                    ...
                                    <div className="d-flex gap-2 mt-4">
                                        ...
                                        <button className={`btn rounded-pill px-4 fw-bold ${projectTab === 'MEMBERS' ? (isProjectClosed ? 'btn-light text-dark' : 'btn-primary') : 'btn-outline-light text-dark bg-white opacity-75'}`} onClick={()=>setProjectTab('MEMBERS')}><i className="bi bi-people-fill me-2"></i>Thành viên ({selectedProject.members?.length || 0})</button>
                                    </div>
                                </div>
                                            ...
                                            <div className="table-responsive bg-white rounded shadow-sm">
                                                <table className="table table-hover align-middle mb-0">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        {tasks.length === 0 && <tr><td colSpan="5" className="text-center py-4 text-muted">Chưa có công việc nào</td></tr>}
                                                    </tbody>
                                                </table>
                                            </div>
                                        </>
            ...
            {showMemberModal && !isProjectClosed && (
                <div className="modal-backdrop-custom"><div className="card shadow-lg border-0" style={{width: 450}}><div className="card-header bg-primary text-white fw-bold d-flex justify-content-between"><span>Thêm nhân viên</span><button className="btn-close btn-close-white" onClick={()=>setShowMemberModal(false)}></button></div><div className="card-body"><select className="form-select mb-3" size="5" value={selectedMemberToAdd} onChange={e => setSelectedMemberToAdd(e.target.value)}>{deptMembers.filter(u => !selectedProject.members?.some(m => m.id === u.id)).map(u => (<option key={u.id} value={u.id}>{u.fullName} ({u.email})</option>))}</select><button className="btn btn-primary w-100 fw-bold" onClick={handleAddMember} disabled={!selectedMemberToAdd}>Xác nhận</button></div></div></div>
            )}
